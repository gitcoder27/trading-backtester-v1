...
class EMA10ScalperStrategyV5(StrategyBase):
    ...
    def generate_signals(self, data):
        ...
        # Dynamic trade sizing (simple version: add 'size' column)
        df['size'] = 1
        if 'pnl' in df.columns:
            for i in range(1, len(df)):
                prev_pnl = df.loc[i-1, 'pnl'] if not pd.isna(df.loc[i-1, 'pnl']) else 0
                if prev_pnl > 0:
                    df.loc[i, 'size'] = min(2, df.loc[i-1, 'size'] + 1)  # Increase size after win
                elif prev_pnl < 0:
                    df.loc[i, 'size'] = max(1, df.loc[i-1, 'size'] - 1)  # Decrease size after loss

        # Volatility regime filter: only trade when ATR is in a reasonable range
        valid_atr_regime = (df['atr'] > self.min_atr) & (df['atr'] < 40)

        # Time-of-day filter: only trade 10 AM–12 PM
        valid_hour_range = (df['trade_hour'] >= 10) & (df['trade_hour'] <= 12) if 'trade_hour' in df.columns else True
        ...
        # Momentum confirmation: price above previous high for long, below previous low for short
        long_condition = (prev_close < prev_ema) & (df['close'] > df['ema']) & valid_time & valid_vol & valid_atr & valid_hour & valid_atr_regime & long_rsi_ok & (df['close'] > prev_high)
        short_condition = (prev_close > prev_ema) & (df['close'] < df['ema']) & valid_time & valid_vol & valid_atr & valid_hour & valid_atr_regime & (df['atr'] > median_atr) & short_rsi_ok & (df['close'] < prev_low)
        long_condition = long_condition & valid_hour_range
        short_condition = short_condition & valid_hour_range
        df['signal'] = 0
    ...
    def should_exit(self, position, row, entry_price, highest_price=None, lowest_price=None):
        ...
        # Adaptive trailing stop: tighten after big move in favor
        if position == 'long' and highest_price is not None and highest_price - entry_price > 2 * atr:
            trailing_stop = 0.5 * self.trailing_stop_mult * atr
        elif position == 'short' and lowest_price is not None and entry_price - lowest_price > 2 * atr:
            trailing_stop = 0.5 * self.trailing_stop_mult * atr
        else:
